...
// 並び順は「深夜」を先頭に＝当日0時台〜3時頃（未明）を各日の最初の枠として扱う
// （夜の続き扱いにすると開催最終日の深夜が開催終了時刻をまたいでしまうため、この並びで統一。cleanSlots はこの順で正規化する）
const SLOTS = ['深夜', '朝', '昼', '夜', '未定'];
...
// SLOTS 外の値は捨て、重複を除き、SLOTS の並び順（深夜→朝→昼→夜→未定）に整える。
